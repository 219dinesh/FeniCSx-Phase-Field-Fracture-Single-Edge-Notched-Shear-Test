...
def initial_crack(x):
    ...
    return np.logical_and(x[0] <= L / 2, np.abs(x[1] - L / 2) < h)
...
# Fixing the left, right and top edge against vertical displacement
...
def psi_0(u):
    return 0.5 * lmbda * (ufl.tr(epsilon(u)))**2 + mu * ufl.inner(epsilon(u), epsilon(u))  # Isotropic Strain Energy Density 

current_psi = fem.Function(V_d)
expr_psi = fem.Expression(psi_0(u), V_d.element.interpolation_points)
...
g_d = (1.0 - d)**2 + k_res
sigma = g_d * (2.0 * mu * epsilon(u) + lmbda * ufl.tr(epsilon(u)) * ufl.Identity(len(u)))
...
Pi_d = ( (Gc / l0) * d * w + Gc * l0 * ufl.inner(ufl.grad(d), ufl.grad(w)) - 2.0 * (1.0 - d) * H * w ) * ufl.dx
...
file_u = XDMFFile(domain.comm, "Fenics/Displacement_Out_line_shear1.xdmf", "w")
file_d = XDMFFile(domain.comm, "Fenics/Damage_Out_line_shear1.xdmf", "w")
...
for step in range(num_steps):
    ...
    with open("Fenics/load_displacement.csv", "a") as f:
